  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-8 md:px-8 lg:py-12">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="w-full lg:grid lg:grid-cols-12 lg:gap-16 xl:gap-24 mb-8">
          {/* תצוגה מקדימה של ההמלצה - בדסקטופ תופיע בצד שמאל */}
          <div className="lg:col-span-7 mb-10 lg:mb-0 lg:order-2">
            <div className="bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden">
              <div className="relative h-56 md:h-72 lg:h-96">
                <img 
                  src={imageUrl || "https://images.unsplash.com/photo-1559925393-8be0ec4767c8?q=80&w=1000&auto=format&fit=crop"}
                  alt={businessName}
                  className="w-full h-full object-cover"
                />
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg text-base font-semibold shadow-sm">
                  {businessName}
                </div>
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent h-24"></div>
              </div>
              <div className="p-6 text-right">
                <p className="text-gray-800 text-lg mb-4 leading-relaxed">{recommendationText}</p>
                <div className="flex items-center justify-end text-sm text-gray-600 border-t border-gray-100 pt-4 mt-4">
                  <div className="flex flex-col items-end">
                    <span className="font-medium">{user?.displayName || "משתמש"}</span>
                    <span className="text-xs text-gray-500">מומלץ לפני {Math.floor(Math.random() * 5) + 1} שעות</span>
                  </div>
                  <img 
            ...
            <div className="hidden lg:grid grid-cols-3 gap-6 mt-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-center">
                <p className="text-2xl font-bold text-gray-900">{Math.floor(Math.random() * 10) + 3}</p>
                <p className="text-gray-600">שמירות</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4 text-center">
                <p className="text-2xl font-bold text-primary-500">{Math.floor(Math.random() * 300) + 50}</p>
                <p className="text-gray-600">נקודות אפשריות</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 lg:order-1 space-y-6">
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
              <h3 className="text-lg font-bold text-blue-800 mb-2 text-right">למה כדאי לשתף?</h3>
              <ul className="text-blue-700 list-disc mr-6 space-y-1 text-right">
                <li>תעזור לחברים שלך לגלות חוויות ועסקים מעולים</li>
                <li>תקבל נקודות בארנק הדיגיטלי על כל שיתוף שמביא צפייה</li>
                <li>תוכל לממש את הנקודות למגוון הטבות בעסקים</li>
                <li>תתרום לקהילה ותעזור לבעלי עסקים מקומיים</li>
              </ul>
            </div>
          </div>
        </div>
